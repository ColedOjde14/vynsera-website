...
export default async function SupportTicketsPage() {
  ...
  const userId = user.id; // This is a string like "user_2abc123..."

  console.log('Current user role:', role);
  console.log('Is admin/support?', isAdminOrSupport);
  ...
  try {
    if (isAdminOrSupport) {
      tickets = await sql`
        ...
        LIMIT 100
      `;
      console.log('Admin fetched tickets:', tickets.length);
    } else {
      tickets = await sql`
        ...
        LIMIT 100
      `;
      console.log('Client fetched tickets:', tickets.length);
    }
  } catch (error) {
