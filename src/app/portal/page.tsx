...
'use client'; // Client component for modal + toast
...
// Client-side wrapper for modal + state
function ClientPortalContent({ displayName, userId }: { displayName: string; userId: string }) {
      ...
      <div className="lg:ml-72">
        ...
        <main className="max-w-7xl mx-auto p-6 sm:p-8 pb-24 lg:pb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: "Submit Support Ticket", color: "indigo", onClick: () => setIsModalOpen(true) },
              ...
              action.onClick ? (
                <button
                  ...
                  className={`bg-gradient-to-r from-${action.color}-900/40 to-${action.color}-900/30 border border-${action.color}-500/30 rounded-2xl p-8 text-left hover:border-white/80 hover:shadow-2xl hover:shadow-white/30 transition-all duration-500 transform hover:scale-105 cursor-pointer`}
                >
