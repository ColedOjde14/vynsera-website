// src/app/request-custom-quote/page.tsx
'use client';

import { useState } from 'react';
import toast from 'react-hot-toast';

export default function RequestCustomQuote() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-gray-950 text-white p-6 sm:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-indigo-300 text-lg mb-12">
          Tell us about your project, budget, and timeline. We'll provide a tailored quote as soon as possible.
        </p>

        <form onSubmit={handleSubmit} className="space-y-6 bg-black/40 backdrop-blur-md border border-indigo-500/30 rounded-2xl p-8">
          <div>
            <label className="block text-indigo-300 text-sm mb-2">Full Name</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-indigo-300 text-sm mb-2">Email Address</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-indigo-300 text-sm mb-2">Phone Number (optional)</label>
            <input
              type="tel"
              value={phone}
          ...
          <div>
            <label className="block text-indigo-300 text-sm mb-2">Service Type</label>
            <select
              value={serviceType}
            ...
            >
              ...
              <option value="custom">Custom Software / SaaS</option>
              <option value="other">Other (describe below)</option>
            </select>
          </div>

          <div>
            <label className="block text-indigo-300 text-sm mb-2">Expected Budget</label>
            <input
              type="text"
              value={budget}
          ...
          <div>
            <label className="block text-indigo-300 text-sm mb-2">Desired Timeline</label>
            <input
              type="text"
              value={timeline}
              onChange={(e) => setTimeline(e.target.value)}
              className="w-full bg-black/50 border border-indigo-500/50 rounded-lg p-4 text-white placeholder:text-indigo-400 focus:outline-none focus:border-indigo-400"
              placeholder="ASAP / 2-4 weeks / 1-3 months / Flexible"
            />
          </div>

          <div>
            <label className="block text-indigo-300 text-sm mb-2">Project Details</label>
            <textarea
              value={details}
              ...
              rows={6}
              className="w-full bg-black/50 border border-indigo-500/50 rounded-lg p-4 text-white placeholder:text-indigo-400 focus:outline-none focus:border-indigo-400"
              placeholder="Describe your project goals, inspiration, timeline, existing assets, or anything else we should know..."
            />
          </div>
